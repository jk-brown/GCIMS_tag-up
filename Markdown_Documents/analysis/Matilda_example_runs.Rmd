...
core_params <- newcore(ini_list[[1]])
...
```{r}
init_result <- lapply(names(ini_list), function(name) {
...
```{r}
scores_co2 <- score_runs(df, criterion = criterion_co2_obs(), score_bayesian)
scores_gmst <- score_runs(df, criterion = criterion_gmst_obs(), score_bayesian)
  list(co2 = scores_co2, gmst = scores_gmst)

# Combine the individual scores into a single list for multi-criteria weighting
mc_scores <- multi_criteria_weighting(scores$ssp370)
```
